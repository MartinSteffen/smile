...
public class PropertiesEntry extends PropertiesPanel{
	...
	public PropertiesEntry(WorkPanel wp, CSMComponent component, Point p) {
	...
	public PropertiesEntry(WorkPanel wp, CSMComponent component, Point p, int error) {
		super(wp, p, component);
		setSize(new Dimension(300,150));
		errorState = error;
		validate();
	...
	void ok() {
		if(name.getText().equals(component.getName()))
			...
			if(wp.isValidName(name.getText())) {
				...
				new PropertiesEntry(wp, component, this.getLocation(), PropertiesPanel.NAMEERROR);
				this.dispose();
			}
	}
	...
	void setWindow() {
		...
		JLabel head = new JLabel("Entry State:");
